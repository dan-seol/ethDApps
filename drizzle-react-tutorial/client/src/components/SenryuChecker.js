...
let checkLineForSyllableCount = (line, n) => {
    ...
    console.warn(`Checking whether the line: "${line.trim()}" has ${n} syllables`)
    let words = line.trim().split(/(\s+)/)
    console.warn(`it has ${words.map(syllable).reduce((prev, curr) => prev + curr)} syllables`,)

    return words.map((word) => {
        console.log(`${word} has ${syllable(word)} syllables`)
